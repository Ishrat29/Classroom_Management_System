...
function RoomQuery() {
    ...
    const onRoomNoChange = (roomNoString) => {
      ...
      Axios.post("http://localhost:3001/room/info", {
        ...
        if(response.data.length===0){
          setIsValidRoom(1);
          setInvalidMessageRoom("Such Room doesnt exist in the given building");
        }else{
          if(editAfterFound!==0){
            setEditAfterFound(0);
  ...
  const roomRoutineToCourseCode = (roomRoutine)=>{
    ...
    //For storing visibility
    const [formClassName, setFormClassName] = useState("form");
    const [filterClassName,setFilterClassName] = useState("filter-non")

    const onRoutineBtnClick = ()=>{
        setFormClassName("form");
        setFilterClassName("filter-non");
        console.log("Clicked R button");
    }

    const onFilterBtnClick = ()=>{
        setFormClassName("form-non");
        setFilterClassName("filter");
        console.log("Clicked F button");
    }

    //Week & time index for filter
  const [weekFilter,setWeekFilter] = useState(0);
  const [timeFilter,settimeFilter] = useState(0);
  const onWeekOption= (event)=>{
    const index = event.target.value.indexOf('.');
    const week = parseInt(event.target.value.substring(0,index))-1;
    setWeekFilter(week);
    console.log("Selected Week: "+week);
  }

  const onTimeOption= (event)=>{
    const index = event.target.value.indexOf('.');
    const week = parseInt(event.target.value.substring(0,index))-1;
    settimeFilter(week);
    console.log("Selected Week: "+week);
  ...
  const [roomLitMessage,setRoomListMessage] = useState("");

  const doQuery =()=>{
    setRoomListMessage("Waiting for Network");
    Axios.post("http://localhost:3001/room/queryVacant",{
        date: weekdays[weekFilter]+"_"+times[timeFilter],
        dateTemp: weekdays[weekFilter]+"_"+times[timeFilter]+"_temp",
    }).then((response)=>{
        console.log(response.data);
        if(response.data.length===0){
            setRoomListMessage("No Vacant Rooms Found :(");
            setRoomList([]);
        }else{
            setRoomListMessage("Vacant Rooms: ");
            console.log(response.data);
            let arr = [];
            for(let i =0;i<response.data.length;i++){
                if(response.data[i][weekdays[weekFilter]+"_"
                +times[timeFilter]]==='|y') arr.push(response.data[i]);
                else if(response.data[i][weekdays[weekFilter]+"_"
                +times[timeFilter+"_temp"]]===('|y'+roomRoutineToDate(response.data[i][weekdays[weekFilter]+"_"
                +times[timeFilter+"_temp"]]).toString())
                &&isValidDate(roomRoutineToDate(response.data[i][weekdays[weekFilter]+"_"
                +times[timeFilter+"_temp"]]))) arr.push(response.data[i]);
                else if(response.data[i][weekdays[weekFilter]+"_"
                +times[timeFilter]]==='|n'&&!isValidDate(roomRoutineToDate(response.data[i][weekdays[weekFilter]+"_"
                +times[timeFilter+"_temp"]]))) arr.push(response.data[i]);
            }
            setRoomList(arr);
        }
      ...
      <>
        ...
        <div className='page-wrap'>
           <div className="col-md-7 col-lg-8">
              ...
              <GenButton buttonStyle="btn--test" onClick={onFilterBtnClick} >
                    {"Get Vacant Rooms at a Specific Time"}
              </GenButton>
              <form className={formClassName}>
                ...
                <div className={isBuildingValid===2?"":"building-invalid"}>
                  ...
                  <div className={(isValidRoom===2||isValidRoom===4)?"main-page":"main-page-non"}>
                    ...
                    <h3>{"Course Code Of Scheduled Course on"+weekdays[weekIndex]+" at "+times[timeIndex]+": "+normalSelectedCourseCode}</h3>
                    <h3>{"Course Code Of temporary Scheduled Course on"+weekdays[weekIndex]+" at "+times[timeIndex]+": "+tempSelectedCourseCode}</h3>
                    <h3>{"Deadline Of temporary Scheduled Course on"+weekdays[weekIndex]+" at "+times[timeIndex]+": "+temporaryDeadline}</h3>
                    <hr></hr>
                    {keyVal}
              ...
              <form className={filterClassName}>
                <select className='building-select' onChange={onWeekOption}>
                  {weekdays.map((it,index)=>(
                    <option key={index} ind={index}>{(index+1).toString()+'. '+it}</option>
                  ))}
                </select>
                <select className='building-select' onChange={onTimeOption}>
                  {times.map((it,index)=>(
                    <option key={index} ind={index}>{(index+1).toString()+'. '+it+" (GMT+6)"}</option>
                  ))}
                </select>
                <GenButton buttonStyle="btn--primary" onClick={doQuery}>
                    {"Search Vacant Rooms"}
                </GenButton>
                {roomLitMessage}
                {roomList.map((it,index)=>(
                    <h3 key={index} ind={index}>{"Room NO.: "+roomList[index]["room_no"]+" & Building: "+roomList[index]["building"]}</h3>
                  ))}
              </form>
            </div>
        </div>
